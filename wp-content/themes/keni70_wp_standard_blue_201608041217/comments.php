 ...
 <?php else : // this is displayed if there are no comments so far ?>
	...
	<?php if ( comments_open() ) : ?>
	 ...
	 <?php else : // comments are closed ?>
		...
		<!--<p class="nocomments"><?php _e('Comments are closed.','keni'); ?></p>-->
...
<h3><?php comment_form_title( __('Leave a Reply', 'keni'), __('Leave a Reply to %s', 'keni') ); ?></h3>
...
<p><?php printf(__('You must be <a href="%s">logged in</a> to post a comment.', 'keni'), wp_login_url( get_permalink() )); ?></p>
<?php else : ?>
...
<p><?php printf(__('Logged in as <a href="%1$s">%2$s</a>.', 'keni'), get_option('siteurl') . '/wp-admin/profile.php', $user_identity); ?> <a href="<?php echo wp_logout_url(get_permalink()); ?>" title="<?php _e('Log out &raquo;','keni'); ?>"><?php _e('Log out &raquo;','keni'); ?></a></p>

<?php else : ?>

<div class="comment-form-author">
<p><label for="author"><small><?php _e('Name','keni'); ?> <span class="required"><?php if ($req) _e('(required)', 'keni'); ?></span></small></label></p>
...
<!--<p><small><?php printf(__('<strong>XHTML:</strong> You can use these tags: <code>%s</code>', 'keni'), allowed_tags()); ?></small></p>-->

<p><textarea name="comment" id="comment" class="w90" cols="58" rows="10" tabindex="4"></textarea></p>

<p class="al-c"><button name="submit" type="submit" id="submit" class="btn btn-form01" tabindex="5" value="<?php _e('Submit Comment', 'keni'); ?>" ><span><?php _e('Submit Comment', 'keni'); ?></span></button>
