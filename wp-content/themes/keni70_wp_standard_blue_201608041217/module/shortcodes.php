...
function newposts_keni_schotcode($atts) {
	extract(shortcode_atts(array('target' => "new", 'rows' => 5, 'excerpt' => 1, 'show_date' => "default", "catid" => 0), $atts));
	$news_data = newposts_keni($target, $rows, $excerpt, $show_date, $catid);
...
function sticky_keni_schotcode($atts) {	
	extract(shortcode_atts(array('target' => "sticky", 'rows' => 3, 'social_counts' => 1, 'excerpt' => 0, 'show_date' => "default", "catid" => 0), $atts));
	$sticky_data = newposts_keni($target, $rows, $social_counts, $excerpt, $show_date, $catid);
...
function sc_character( $atts, $content = null ) {
	...
	if (preg_match("/^[0-9]+$/", $no) and ($no > 0)) {
		$character = $wpdb->get_results($wpdb->prepare( "SELECT * FROM ".$wpdb->prefix."keni_character WHERE kc_id=%d", $no), ARRAY_A);
		...
		$character = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix."keni_character WHERE kc_id IN (".$no.") ORDER BY field(kc_id,".$no.")", ARRAY_A);
...
//	共通コンテンツの表示
//---------------------------------------------------------------------------
function get_common_contents ($atts, $comm_post_id) {
	...
	if (isset( $content['post_content']) && $content['post_status'] == "publish") {
		return do_shortcode(richtext_formats($content['post_content']));
	} else {
		return "";
	}
}
