...
//---------------------------------------------------------------------------
global $wpdb;
define("KENI_CHAR",$wpdb->prefix."keni_character");


$table_alive = $wpdb->get_results("SHOW TABLES LIKE '".KENI_CHAR."'");

if (!isset($table_alive) or count($table_alive) <= 0) {
	if ($wpdb->get_var("show tables like '".KENI_CHAR."'") != KENI_CHAR) {
		$char = defined("DB_CHARSET") ? DB_CHARSET : "utf8";
		$sql = "CREATE TABLE ".KENI_CHAR." (
...
function viewList() {
	...
	// リストを取得
	$sql = "SELECT * FROM ".KENI_CHAR." WHERE kc_active='y' ORDER BY kc_id";
	...
	echo "<div class=\"wrap\">";
	echo "<h2>".__('Character Setting','keni') ."<a href=\"admin.php?page=keni_character_add\" class=\"add-new-h2\">".__('Character Registration','keni')."</a></h2>\n";
	if (isset($list) && count($list)> 0) {
		echo "<table class=\"wp-list-table widefat fixed striped posts\" id=\"char-list\">\n";
		echo "<thead>\n<tr>\n<th class=\"manage-column column-id\">ID</th>\n<th class=\"manage-column column-name\">".__('Name','keni') ."</th>\n<th class=\"manage-column\">プレビュー</th>\n<th class=\"manage-column\">ショートコード</th>\n</tr>\n</thead>\n";
		foreach ($list as $kc_id => $val) {		
			$position = ($val['kc_position'] == "right") ? "chat-r" : "chat-l";
			echo "<tbody>\n<tr>\n<td>".$kc_id."</td>\n<td><a href=\"admin.php?page=keni_character&amp;kc_id=".$kc_id."\">".esc_html($val['kc_name'])."</a></td>\n<td>";
...
function regView($error=null, $data) {
	...
	echo (isset($data['kc_color'])) ?  "<script>\ndefault_color='".$data['kc_color']."';\n</script>\n" : "<script>\ndefault_color='ffffff';\n</script>\n";

	$kc_name = (isset($data['kc_name'])) ? $data['kc_name'] : "";
	$kc_image = (isset($data['kc_image'])) ? $data['kc_image'] : "";
	...
	echo "<tr>\n<th>名前</th>\n<td><input type=\"text\" name=\"kc_name\" value=\"".esc_html($kc_name)."\" />";
	if (isset($error['kc_name'])) echo "<p class=\"keni_note\">".esc_html($error['kc_name'])."</p>";
	echo "</td>\n</tr>\n";
	...
	echo "<tr>\n<th>画像</th>\n<td><p id=\"keni_img_1\">";
	if ($kc_image != "") echo "<img src=\"".esc_html($kc_image)."\" />";
	echo "</p>\n";
	echo "<input id=\"keni_upload_image_1\" type=\"text\" size=\"80\" name=\"kc_image\" value=\"". esc_html($kc_image) ."\" /><br />";
	echo "<p><input type=\"button\" class=\"keni_upload_image_button\" id=\"upload_image_button_1\" value=\"画像を設定する\" /></p>\n";
	if (isset($error['kc_image'])) echo "<p class=\"keni_note\">".esc_html($error['kc_image'])."</p>\n";
	echo "</td>\n</tr>\n";
	...
	if (isset($error['kc_image_style'])) echo "<p class=\"keni_note\">".esc_html($error['kc_image_style'])."</p>";
	...
	if (isset($error['kc_balloon'])) echo "<p class=\"keni_note\">".esc_html($error['kc_balloon'])."</p>";
	echo "</td>\n</tr>\n";
	...
	if (isset($error['kc_color'])) echo "<p class=\"keni_note\">".esc_html($error['kc_color'])."</p>";
	echo "</td>\n</tr>\n";
	...
	if (isset($error['kc_position'])) echo "<p class=\"keni_note\">".esc_html($error['kc_position'])."</p>";
	echo "</td>\n</tr>\n";
	...
	if (isset($_GET['page']) && $_GET['page'] == "keni_character" && isset($_GET['kc_id']) && preg_match("/^[0-9]+/", $_GET['kc_id'])) {
...
function regCharacter() {
	...
	if (isset($error)) {
		...
		// データベースに入れる
		global $wpdb;
		$wpdb->query($wpdb->prepare( "INSERT INTO ".KENI_CHAR." ( kc_name, kc_name_view, kc_image, kc_image_style, kc_balloon, kc_color, kc_position, kc_button_view ) VALUES ( %s, %s, %s, %s, %s, %s, %s, %s )",  $_POST['kc_name'], $_POST['kc_name_view'], $_POST['kc_image'], $_POST['kc_image_style'], $_POST['kc_balloon'], $_POST['kc_color'], $_POST['kc_position'], $_POST['kc_button_view']));
		unset($_POST);
...
function delCharacter() {
	global $_POST;
	global $wpdb;
	$res = $wpdb->query($wpdb->prepare( "UPDATE ".KENI_CHAR." SET kc_active=%s WHERE kc_id=%d",  "n", $_POST['kc_id']));
	unset($_POST);
	return false;
}
...
function getCharacter($kc_id="") {
	if (preg_match("/^[0-9]+$/", $kc_id) and ($kc_id > 0)) {
		global $wpdb;
		$character = $wpdb->get_results($wpdb->prepare( "SELECT * FROM ".KENI_CHAR." WHERE kc_id=%d AND kc_active='y'", $kc_id), ARRAY_A);
